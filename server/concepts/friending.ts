...
export default class FriendingConcept {
  ...
  async addFriend(requester: ObjectId, receiver: ObjectId) {
    const existing = await this.requests.readOne({ from: requester, to: receiver });
    if (existing != null) {
      throw new NotAllowedError(`User ${requester} has already added ${receiver} as a friend`);
    }
    await this.requests.createOne({ from: requester, to: receiver });
    return { msg: `Sent request!` };
  }

  async assertFriends(friend1: ObjectId, friend2: ObjectId) {
    const one = this.requests.readOne({ from: friend1, to: friend2 });
    const two = this.requests.readOne({ from: friend2, to: friend1 });
    if (!one || !two) {
      throw new NotAllowedError(`${friend1} and ${friend2} are not friends`);
    }
  }

  async removeFriend(requester: ObjectId, receiver: ObjectId) {
    const existing = await this.requests.readOne({ from: requester, to: receiver });
    if (!existing) {
      throw new NotAllowedError(`${requester} has not added ${receiver} as a friend`);
    }
    await this.requests.deleteOne({ from: requester, to: receiver });
    return { msg: "Removed friend!" };
  }

  async getAddedFriends(user: ObjectId) {
    const requestList = await this.requests.readMany({ from: user });
    const addedFriends = requestList.map((reqDoc) => reqDoc.to);
    return addedFriends;
  }
